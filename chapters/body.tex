...
\section{表达式}
  ...
  \footnote{此处 \texttt{\string\eqref} 使用中文括号以保持统一。}
...
\makeatother
  ...
  \subsection{分类别示例}\label{subsec:bib_types}
    ...
    \textbf{会议论文 [C]}：\parencite{zhao2023automatic,huang2023low}；
...
\section{章节}
  ...
  \subsection{交叉引用}
    使用 \texttt{\string\ref} 引用一节，
    例如 \texttt{\string\S\string\ref\{sec:draw\_color\}}
    可以获得 \S\ref{sec:draw_color}。
    由于章节数字为中文，可以使用宏 \texttt{\string\ChapRef}，
    例如\ChapRef{chap:body}（请注意前后空格）。
