\documentclass[degree = m, fontset = mac ms]{seuthesis2024} % 使用 macOS 上的 MS Word 字体
% \documentclass[degree = m]{seuthesis2024} % 使用 fonts 文件夹字体
% \documentclass[degree = m, twoside]{seuthesis2024} % 使用双页模式
% 更多自定义选项见 seuthesis2024.cls 文件的定义
% 注意选项可以合并使用
...
\begin{document}
  ...
  \input{chapters_m/introduction}
    
  \chapter{正文}\label{chap:body}
  \input{chapters_m/body}

  % \chapter{模板使用}\label{chap:usage}
  % \input{chapters_b/usage}
  ...
  % \input{chapters_b/appendix}
  
  % 致谢
  \anony{% hidden in the anonymous version
    \chapterAck
    \input{chapters_m/acknowledgment}%
  }{}

  \chapterBio
  \input{chapters_m/biography}
